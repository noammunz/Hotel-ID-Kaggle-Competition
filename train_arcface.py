...
in_features = backbone.fc.in_features  # Store the in_features value
backbone.fc = nn.Identity()  # Remove the final fully connected layer
arcface  = ArcFace(in_features=in_features, out_features=num_classes)
model = ArcFaceModel(backbone, arcface).to(device)
